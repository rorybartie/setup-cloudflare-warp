...
async function writeMacOSConfiguration(
  ...
  await exec.exec('sudo mkdir -p "/Library/Application Support/Cloudflare/"');
  await fs.writeFile("/tmp/mdm.xml", config);
  await exec.exec(
    'sudo mv /tmp/mdm.xml "/Library/Application Support/Cloudflare/"',
  );
}
...
export async function cleanup() {
  ...
  switch (process.platform) {
    ...
    case "darwin":
      ...
      await exec.exec(
        'sudo rm "/Library/Application Support/Cloudflare/mdm.xml"',
      );
      break;
    case "win32":
